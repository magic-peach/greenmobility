...
export function RideSearchPage({ context }: RideSearchPageProps) {
  ...
  const handleJoinRide = async (ride: Ride) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-1659ed12/rides/${ride.id}/join`,
        {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            pickup_location_name: ride.start_location_name,
            pickup_lat: 0,
            pickup_lng: 0,
            drop_location_name: ride.end_location_name,
            drop_lat: 0,
            drop_lng: 0,
            distance_km: 0,
            fare_share: ride.estimated_fare || 0,
          }),
        }
      );
